...
Scan the QR code from the SimPilot desktop app and start playing.

## Download

Go to the [latest release](https://github.com/jerenders/simpilot-releases/releases/latest) and download the installer for your system:

| File | Who it's for |
|------|-------------|
| **SimPilot-Setup-x64.exe** | Most Windows PCs (Intel / AMD) — **download this one** |
| SimPilot-Setup-arm64.exe | ARM-based Windows PCs (Surface Pro X, etc.) |

> The `.blockmap`, `latest.yml`, and source code files are used internally for auto-updates. You don't need to download them.

## Installation

SimPilot is currently in beta and not yet code-signed, so Windows will show a few warnings during installation. This is normal for beta software — here's exactly what to expect.

### Step 1: Download

- Click the **x64** installer link from the [latest release](https://github.com/jerenders/simpilot-releases/releases/latest)
...
- On first launch, Windows Firewall will ask: *"Allow SimPilot to communicate on networks?"*
- Click **"Allow access"** — this lets your phone connect over WiFi

### Step 4: Connect Your Phone

- SimPilot opens and shows a **QR code**
- Scan it with your phone's camera
- The remote control opens in your phone's browser — no app install needed
- Both devices must be on the **same WiFi network**

> **These warnings only happen once.** After installation, SimPilot starts cleanly from the Start Menu or system tray. Future updates install automatically with no warnings.

## Security

SimPilot runs entirely on your local network — no data is sent to external servers. All control signals stay between your phone and your PC over WiFi.

This installer has been scanned by 70+ antivirus engines on VirusTotal:

- **v1.2.0:** [View scan results](https://www.virustotal.com/gui/file/cb4ddbb9c54d6c1ce6eadcb4c205b191f7288475a4a1a8cc2ff3dacf08743621/detection)
...
SimPilot includes a **14-day free trial** that starts automatically when you install it.
...
Once installed, SimPilot checks for updates automatically on every launch. When a new version is available:
...
- **Phone can't connect?** Make sure you clicked "Allow" on the Windows Firewall prompt. If you missed it, search Windows for "Allow an app through firewall" and enable SimPilot.
...
- **App not starting?** Check that SimPilot isn't already running in the system tray (bottom-right of your taskbar).
